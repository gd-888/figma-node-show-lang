...
`;
  ...
  figmaVariables.processNodeVariables = figmaVariables.getNodeIdVariable = void 0;
  /**
   * 获取节点变量映射
   ...
   */
  figmaVariables.processNodeVariables = (checkNode, nodeIdVariables, variables) => {
      // 参数验证
      ...
      if (nodeIdVariables[checkNode.id]) {
          ...
          if (nodeIdVariable.opacity && variables[nodeIdVariable.opacity.id]) {
              checkNode.style.opacity =
                  `var(--${variables[nodeIdVariable.opacity.id].name})`;
          }
          // 处理圆角变量
          if (nodeIdVariable.rectangleCornerRadii) {
              const cornerRadii = nodeIdVariable.rectangleCornerRadii;
              // 左上圆角
              if (cornerRadii.RECTANGLE_TOP_LEFT_CORNER_RADIUS &&
                  variables[cornerRadii.RECTANGLE_TOP_LEFT_CORNER_RADIUS.id]) {
                  checkNode.style.borderTopLeftRadius = `var(--${variables[cornerRadii.RECTANGLE_TOP_LEFT_CORNER_RADIUS.id].name})`;
              }
              // 右上圆角
              if (cornerRadii.RECTANGLE_TOP_RIGHT_CORNER_RADIUS &&
                  variables[cornerRadii.RECTANGLE_TOP_RIGHT_CORNER_RADIUS.id]) {
                  checkNode.style.borderTopRightRadius = `var(--${variables[cornerRadii.RECTANGLE_TOP_RIGHT_CORNER_RADIUS.id].name})`;
              }
              // 左下圆角
              if (cornerRadii.RECTANGLE_BOTTOM_LEFT_CORNER_RADIUS &&
                  variables[cornerRadii.RECTANGLE_BOTTOM_LEFT_CORNER_RADIUS.id]) {
                  checkNode.style.borderBottomLeftRadius = `var(--${variables[cornerRadii.RECTANGLE_BOTTOM_LEFT_CORNER_RADIUS.id].name})`;
              }
              // 右下圆角
              if (cornerRadii.RECTANGLE_BOTTOM_RIGHT_CORNER_RADIUS &&
                  variables[cornerRadii.RECTANGLE_BOTTOM_RIGHT_CORNER_RADIUS.id]) {
                  checkNode.style.borderBottomRightRadius = `var(--${variables[cornerRadii.RECTANGLE_BOTTOM_RIGHT_CORNER_RADIUS.id].name})`;
              }
          }
      }
  };
   ...
   * 添加组件依赖的辅助函数
   */
  const addComponentDependency = (checkNode, componentName, packageName, defaultModule = false) => {
  ...
  const processImg = (checkNode) => {
      ...
      if (checkNode.name.startsWith('RcImgLang')) {
          ...
          // 2️⃣ 新增依赖
          addComponentDependency(checkNode, 'MultilingualImage', '@/core/components/language/MultilingualImage', false);
      }
      else {
          checkNode.tag = 'BlobImage';
          // 2️⃣ 新增依赖
          addComponentDependency(checkNode, 'BlobImage', '@/core/components/BlobImage', false);
      }
      // 正确返回替换后的节点
      return checkNode;
  };
  ...
  const processButton = (checkNode) => {
      ...
      // 2️⃣ 新增依赖
      addComponentDependency(checkNode, 'Button', '@/core/components/button/Button', false);
  ...
  modifyJSONSchema.modifyJSONSchema = async (node, pluginOptions) => {
      ...
      if (variablesData) {
          ...
          const figmaData = await figmaApi_1.getFileNodes(pluginOptions.figmaToken, variablesData.figmaFileId, [figmaNodeId]);
          console.log('figmaData', figmaData.nodes[node.nodeId].document);
          nodeIdVariables = figmaVariables_1.getNodeIdVariable(figmaData.nodes[node.nodeId].document);
      ...
      const checkRc = (checkNode) => {
          // 处理节点变量样式
          figmaVariables_1.processNodeVariables(checkNode, nodeIdVariables, variables);
          if (checkNode.style.backgroundImage &&
          ...
          options: [
              {
                  name: 'figmaToken',
                  type: 'input',
                  defaultValue: 'figd_iOdMoRBZtbtNDPt3vAbSEFy0gUiaYCQslTh_F6TD',
              },
          ],
          setup(api, pluginOptions) {
              api.modifyJSONSchema(async (node) => {
                  const { newNode, variablesData } = await modifyJSONSchema_1.modifyJSONSchema(node, pluginOptions);
